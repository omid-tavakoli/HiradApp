...
const KindDetailsTab = ({ projectId, kindId }) => {
  ...
  const onEdit = async (formData, id) => {
    const dataSent = formData?.testKindDetailDtos?.map((item) => ({
      title: item?.title,
    }));
    try {
  ...
  const handleAddKindDetailsToProject = async () => {
    ...
    try {
      if (addedKindDetails.length == 0) {
        ...
        const response = await request.apiCall(
          "post",
          `Project/EditKindDetailProject?projectId=${projectId}`,
          checkedItems
        );
        if (response?.isSuccess) {
          getListData();
          getListAddedKindDetails();
          toast.success(`سربرگ های ${user?.role?.listSystemSet?.filter(item => item.number == 2 )[0]?.fieldValue ? user?.role?.listSystemSet?.filter(item => item.number == 2 )[0]?.fieldValue: 'پروژه'} با موفقیت ویرایش شد`);
        }
      }
    } catch (error) {
      console.log("ERR", error);
  ...
  return (
    <>
      ...
      عنوان سربرگ های مورد نظر خود را برای افزودن به {user?.role?.listSystemSet?.filter(item => item.number == 2 )[0]?.fieldValue ? user?.role?.listSystemSet?.filter(item => item.number == 2 )[0]?.fieldValue: 'پروژه'} وارد کنید
      </span>
