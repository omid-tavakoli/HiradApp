...
import AddSupervisorsForm from "../KindCreateStepper/forms/AddSupervisorsForm";
import useApi from "../../../../hooks/useApi";
import BeatLoaderLoading from "../../../element/loading/BeatLoader";
import { useUser } from "../../../../contexts/UserContext";

const SupervisorsTab = ({ kindId }) => {
  ...
  const onAdd = async (data) => {
    try {
      ...
      if (response?.isSuccess) {
        getListData();
        setShowModal(false);
        toast.success(`${user?.role?.listSystemSet?.filter(item => item.number == 5 )[0]?.fieldValue ? user?.role?.listSystemSet?.filter(item => item.number == 5 )[0]?.fieldValue : 'ناظر'} با موفقیت اضافه شد`);
      }
    } catch (error) {
    ...
    setModalContent({
      ...
      children: (
        <AddSupervisorsForm
          ...
          defaultValues={{
            ...targetItem,
            userId: { label: targetItem?.name, value: targetItem?.userId },
          }}
        />
      ),
    });
